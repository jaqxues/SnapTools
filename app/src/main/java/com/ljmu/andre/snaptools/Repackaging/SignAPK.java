...
public class SignAPK {
    ...
    public static void signZip(X509Certificate certificate, PrivateKey privateKey,
        ...
        try {
            try (OutputStream out = new BufferedOutputStream(new FileOutputStream(temp1))) {
                signZip(certificate, privateKey, input, out, false);
            }

            ZipAdjust.adjust(temp1, temp2);

            try (JarMap map = new JarMap(temp2, false)) {
                signZip(certificate, privateKey, map, output, true);
            }
        } finally {
            temp1.delete();
            temp2.delete();
